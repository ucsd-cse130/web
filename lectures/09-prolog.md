...
### Prolog Terms Generated By Grammar
...
term     := const | atom | variable | atom(term,term,term,...)
~~~~~
-->
...
<! -- - Only thing prolog knows: `factorial(X) = factorial(X)`
-->
...
As when it **plugs-in** `kim` for `X`, it can infer `parent(margaret,
kim)`.
